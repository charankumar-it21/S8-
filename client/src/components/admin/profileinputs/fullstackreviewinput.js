...
export default function Table() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/protected/get-achievements", {
        headers: {
  ...
  const handleEdit = (editedItem) => {
    setData(
      data.map((item) => (item.roll_no === editedItem.roll_no ? editedItem : item))
    );
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
              ...
              <th className="py-3 px-6 text-left whitespace-nowrap">Achievement Name</th>
              ...
              <th className="py-3 px-6 text-left whitespace-nowrap">Award Level</th>
              <th className="py-3 px-6 text-left whitespace-nowrap">Date Awarded</th>
          ...
          <tbody className="text-gray-600 text-md font-normal">
            {paginatedData.map((item, index) => (
              <tr key={item.roll_no} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-3 px-6 text-left whitespace-nowrap">{item.achievement_name}</td>
                ...
                <td className="py-3 px-6 text-left whitespace-nowrap">{item.award_level}</td>
                <td className="py-3 px-6 text-left whitespace-nowrap">{item.date_awarded}</td>
                <td className="py-3 px-6 text-center">
                  <div className="flex item-center justify-center">
                    <button className="transform text-[#6777EF] mr-3" onClick={() => {
                      setCurrentItem(item);
